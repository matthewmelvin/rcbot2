		...
		if ((args[0] && *args[0]) && (args[1] && *args[1]))
		{
			...
			if (edict_t* pNearest = CClassInterface::FindEntityByNetClassNearest(pClient->getOrigin(), args[0]))
			{
				...
				if (const ServerClass* sc = UTIL_FindServerClass(args[0]))
				{
					...
					if (m_offset)
					{
						IServerUnknown* pUnknown = pNearest->GetUnknown();
						...
						if (void* data = reinterpret_cast<char*>(pEntity) + m_offset)
						{
							...
							CBotGlobals::botMessage(pPlayer, 0, "int = %d, float = %f, bool = %s, Vector = (%0.4f,%0.4f,%0.4f)",
								*(static_cast<int*>(data) + preoffs),
								*(static_cast<float*>(data) + preoffs),
								(*(static_cast<bool*>(data) + preoffs) ? "true" : "false"),
...
CBotCommandInline FindClassname("findclassname", CMD_ACCESS_DEBUG, [](const CClient* pClient, const BotCommandArgs& args)
{
	if (pClient)
	{
		if (args[0] && *args[0])
		{
			...
			{
				CBotGlobals::botMessage(pClient->getPlayer(), 0, "%s network name not found", args[0]);
			}
		}
...
CBotCommandInline DebugMemoryScanCommand("memoryscan", CMD_ACCESS_DEBUG, [](const CClient* pClient, const BotCommandArgs& args)
{
	...
	for (u_MEMSEARCH& stored_offset : stored_offsets)
	{
		bool bfound = false;
...
CBotCommandInline DebugMemoryCheckCommand("memorycheck", CMD_ACCESS_DEBUG, [](const CClient* pClient, const BotCommandArgs& args)
{
	NEED_ARG(args[0])
	NEED_ARG(args[1])
	NEED_ARG(args[2])
