...
bool CDODBot :: setVisible ( edict_t *pEntity, const bool bVisible )
{
		...
		if ( (fSmokeTime >= 1.0f) && (fSmokeTime <= 10.0f) )
		{
			...
			else if ( m_pEnemy && hasSomeConditions(CONDITION_SEE_CUR_ENEMY) )
			{
					...
					(distanceFrom(pEntity) < (distanceFrom(m_pEnemy)+SMOKE_RADIUS)) )
				{
					...
					if ( m_pNearestSmokeToEnemy && (std::fabs(DotProductFromOrigin(CBotGlobals::entityOrigin(pEntity))-
							  DotProductFromOrigin(CBotGlobals::entityOrigin(m_pEnemy))) <=
						 std::fabs(DotProductFromOrigin(CBotGlobals::entityOrigin(m_pNearestSmokeToEnemy))-
							  DotProductFromOrigin(CBotGlobals::entityOrigin(m_pEnemy)))) )
					{
						m_pNearestSmokeToEnemy = pEntity;
					}
	...
	else if ( pEntity == m_pNearestSmokeToEnemy )
	{
		...
		if ( bNoDraw || ((fSmokeTime < 1.0f) || (fSmokeTime > rcbot_smoke_time.GetFloat())) )
			m_pNearestSmokeToEnemy = nullptr;
	}
		...
		if ( !hasEnemy() )
		{
			...
			if ( !m_pSchedules->isCurrentSchedule(SCHED_INVESTIGATE_NOISE) )
			{
				ADD_UTILITY_DATA_VECTOR(
					...
					engine->IndexOfEdict(pDied), m_vListenPosition)
...
void CDODBot :: touchedWpt ( CWaypoint *pWaypoint, const int iNextWaypoint, const int iPrevWaypoint )
{
	...
	else
	{
		...
		if ( (iPrevWaypoint != -1) && (iNextWaypoint != -1) && (randomFloat(0,MAX_BELIEF) < m_pNavigator->getBelief(CWaypoints::getWaypointIndex(pWaypoint))) )
		{
			...
			{
				std::size_t iCheck = static_cast<std::size_t>(randomInt(0, static_cast<int>(m_InvisPaths.size() - 1)));

				iCheck = static_cast<std::size_t>(m_InvisPaths[iCheck]);
				...
				if ( !m_pSchedules->hasSchedule(SCHED_INVESTIGATE_HIDE) )
				{
					...
					pSched->addTask(new CBotInvestigateHidePoint(static_cast<int32_t>(iCheck), CWaypoints::getWaypointIndex(pWaypoint)));
...
void CDODBot :: modThink ()
{
	...
	if ( m_bCheckClass && !m_pPlayerInfo->IsDead())
	{
		...
		if ( bot_change_class.GetBool() && (m_fChangeClassTime < engine->Time()) )
		{
				...
				const float scoreValue = CDODMod::getScore(m_pEdict);
	...
	if ( m_pNearestPathBomb )
	{
		...
		if ( CClassInterface::getDODBombState(m_pNearestPathBomb) == 2 )
		{
			if ( distanceFrom(m_pNearestPathBomb) < (BLAST_RADIUS*2) )
			{
				...
				{
					if ( CWaypoint *pWaypoint = CDODMod::getBombWaypoint(m_pNearestPathBomb) )
					{
	...
	if ( m_pNearestBomb )
	{
		...
		{
			const Vector vNearestBomb = CBotGlobals::entityOrigin(m_pNearestBomb);
			...
			if ( distanceFrom(vNearestBomb) < (BLAST_RADIUS*2) )
			{
				...
				{
					if ( CWaypoint *pWaypoint = CDODMod::getBombWaypoint(m_pNearestBomb) )
					{
	...
	case DOD_VC_GO_RIGHT:
		IF_WANT_TO_LISTEN
		{
			...
			if ( p )
			{
				...
				CWaypoint *pWaypoint = CWaypoints::getWaypoint(CWaypointLocations::NearestWaypoint(vOrigin,768.0f,-1,false,false,false, nullptr,false,0,false,false,Vector(0,0,0),CWaypointTypes::W_FL_ROUTE, nullptr));
				...
				if ( pWaypoint != nullptr)
				{
					if ( iNearestPoint != -1 )
					{
						...
						if ( pCap != nullptr) // go here after going through route
				...
				else
				{
					...
					CWaypoint *waypoint = CWaypoints::getWaypoint(CWaypointLocations::NearestWaypoint(vOrigin,768.0f,-1,false,false,false, nullptr,false,0,false,false,Vector(0,0,0),CWaypointTypes::W_FL_CAPPOINT, nullptr));

					if ( waypoint != nullptr)
					{
						m_pSchedules->addFront(new CBotGotoOriginSched(waypoint->getOrigin()));
						addVoiceCommand(DOD_VC_YES);
					}					
					else
						addVoiceCommand(DOD_VC_NO);
		...
		if ( !m_bDroppedAmmoThisRound )
		{
			...
			if ( (!inSquad() && (distanceFrom(pPlayer)<512.0f) && isVisible(pPlayer)) || (inSquad()&&m_pSquad->IsMember(pPlayer)) )
			{
				if ( !m_pSchedules->isCurrentSchedule(SCHED_DOD_DROPAMMO) )
				{
					...
					{
						if ( CWaypoint *pWpt = CWaypoints::getWaypoint(CWaypointLocations::NearestWaypoint(CBotGlobals::entityOrigin(pPlayer),200.0f,-1)) )
						{
...
bool CDODBot :: withinTeammate () const
{
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i ++ )
	{
		...
		{
			if ( IPlayerInfo* playerinfo = playerinfomanager->GetPlayerInfo(pPlayer) )
			{
...
bool CDODBot :: executeAction ( CBotUtility *util )
{
	...
	case  BOT_UTIL_INVESTIGATE_POINT:
	{
		...
		m_pSchedules->removeSchedule(SCHED_INVESTIGATE_NOISE);
		m_pSchedules->addFront(new CBotInvestigateNoiseSched(CBotGlobals::entityOrigin(edict), util->getVectorData()));
		return true;
	}
	case BOT_UTIL_COVER_POINT:
	{
		...
		m_pSchedules->removeSchedule(SCHED_CROUCH_AND_HIDE);
		m_pSchedules->addFront(new CCrouchHideSched(edict));
		return true;
	}
	case BOT_UTIL_SNIPE_POINT:
		// find sniper point facing the enemy
		{
			...
			if (!pEnemy || pEnemy->IsFree() || pEnemy->GetIServerEntity() == nullptr)
			{
				return false;
			}
	...
	case BOT_UTIL_DEFEND_NEAREST_POINT:
		{
			Vector v_goal;
			int i_area = util->getIntData();
			bool defend_wpt = true;

			v_goal = util->getVectorData();

			if ( inSquad() && !isSquadLeader() )
			{
				if ( (m_pSquad->GetFormationVector(m_pEdict)-v_goal).Length() > 400.0f )
					return false;
			}

			CWaypoint *pWaypoint;

			pWaypoint = CWaypoints::randomWaypointGoal(CWaypointTypes::W_FL_DEFEND,m_iTeam,i_area,true,this);

			if ( pWaypoint == nullptr)
			{			
				defend_wpt = false;

				if ( distanceFrom(v_goal) > 1024 ) // outside waypoint bucket of goal
					pWaypoint = CWaypoints::getPinchPointFromWaypoint(v_goal,v_goal);
				else
					pWaypoint = CWaypoints::getPinchPointFromWaypoint(getOrigin(),v_goal);
			}
			...
			if ( pWaypoint )
			{
				...
				CBotTask *deftask = new CBotDefendTask(pWaypoint->getOrigin(),randomFloat(7.5f,12.5f),0,true,v_goal,defend_wpt ? LOOK_SNIPE : LOOK_AROUND,pWaypoint->getFlags());
	...
	case BOT_UTIL_ATTACK_NEAREST_POINT:
		{
			...
			attack->addTask(new CBotDODAttackPoint(iFlagID,v_goal,150.0f));
			// add defend task
			m_pSchedules->add(attack);
			removeCondition(CONDITION_PUSH);
	...
	case BOT_UTIL_SNIPE: // find snipe or machine gun waypoint
		{
			...
			if ( inSquad() && !isSquadLeader() )
			{
				...
				if ( iFlagID == -1 )
				{
					pWaypoint = CWaypoints::getWaypoint(CWaypointLocations::NearestWaypoint(
						m_pSquad->GetFormationVector(m_pEdict), 250.0f, -1, false, false, true, nullptr, false, m_iTeam,
						false, false, Vector(0, 0, 0), iWaypointType));					
				}
				else
			...
			else
			{
				if ( util->getIntData() ) // attack
				{
					...
					{
						if ( CDODMod::m_Flags.getRandomEnemyControlledFlag(this,&v_goal,getTeam(),&iFlagID) )
						{
							pWaypoint = CWaypoints::randomWaypointGoal(iWaypointType,m_iTeam,iFlagID,true,this);
						}
					}
				...
				{
					if ( CDODMod::m_Flags.getRandomTeamControlledFlag(this,&v_goal,getTeam(),&iFlagID) )
					{
						pWaypoint = CWaypoints::randomWaypointGoal(iWaypointType,m_iTeam,iFlagID,true,this);
					}
				}
			...
			if ( pWaypoint )
			{
				...
				{
					if ( CWaypoint *pBombs = CWaypoints::randomWaypointGoal(CWaypointTypes::W_FL_BOMBS_HERE,m_iTeam,pWaypoint->getArea(),true) )
					{
						iFlagID = pWaypoint->getArea();
						v_goal = pBombs->getOrigin();
					}
				}

				snipetask = new CBotDODSnipe(util->getWeaponChoice(),pWaypoint->getOrigin(),pWaypoint->getAimYaw(),iFlagID!=-1,v_goal.z+48,pWaypoint->getFlags());
	...
	case BOT_UTIL_DEFEND_NEAREST_BOMB:
		...
		[[fallthrough]]; // tell the compiler that fallthrough is the intended behavior here. (C++17 feature) -caxanga334
	...
	case BOT_UTIL_DEFEND_POINT:
		{
			...
			{
				if ( !CDODMod::m_Flags.getRandomTeamControlledFlag(this,&vGoal,getTeam(),&i_area) )
					return false;
			}
		...
		if ( CWaypoint *pWaypoint = CWaypoints::getWaypoint(CWaypointLocations::GetCoverWaypoint(getOrigin(),m_vLastSeeEnemy, nullptr)) )
			{
				...
				pSched->addTask(new CThrowGrenadeTask(pWeapon, getAmmo(static_cast<std::size_t>(pWeapon->getWeaponInfo()->getAmmoIndex1())), m_vLastSeeEnemyBlastWaypoint)); // first - throw
	...
	case BOT_UTIL_ATTACK_POINT:
		{
			...
			if ( CDODMod::m_Flags.getRandomEnemyControlledFlag(this,&v_goal,getTeam(),&iFlagID) )
			{
				...
				if ( (pRoute == nullptr) && (randomFloat(0.0f,MAX_BELIEF) < m_pNavigator->getBelief(iGoalWaypoint)) )
				{
					CWaypoint *pWaypoint;
					if ( distanceFrom(v_goal) > 1024 ) // outside waypoint bucket of goal
						pWaypoint = CWaypoints::getPinchPointFromWaypoint(v_goal,v_goal);
...
bool CDODBot:: checkStuck ()
{
	if ( CBot::checkStuck() )
	{
		...
		if ( const edict_t *pGroundEnt = CClassInterface::getGroundEntity(m_pEdict) ) // stuck on furniture? 
		{
...
void CDODBot :: getTasks (unsigned iIgnore)
{
	...
	if ( CDODMod::isFlagMap() && (CDODMod::m_Flags.getNumFlags() > 0) && !rcbot_nocapturing.GetBool() )
	{
		...
		if ( inSquad() )
		{
			...
			{
				if ( CWaypoint *pWaypoint = CWaypoints::getWaypoint(CDODMod::m_Flags.getWaypointAtFlag(iFlagID)) )
				{
	...
	if ( CDODMod::isBombMap() && (CDODMod::m_Flags.getNumFlags() > 0) && !rcbot_nocapturing.GetBool() )
	{
		...
		{
			const Vector vBomb = CBotGlobals::entityOrigin(m_pNearestBomb);
